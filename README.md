...
- **13 Major Clinical Categories**: Cardiovascular, Infectious Diseases, Endocrinology, and more.
...
├── admin/
│   ├── index.html         # Admin dashboard HTML shell
│   └── admin.js           # Admin dashboard logic (CSP-safe external script)
...
npm run smoke:mem       # Run API memory drift check (leak guard)
npm run security:audit  # Run dependency vulnerability audit
npm run security:outdated # Show available dependency updates
...
- [TRAINING_HANDOFF_LOG.md](TRAINING_HANDOFF_LOG.md)

Windows maintenance scripts:
- `Quiz/backend/setup-scheduled-backup.ps1` (creates daily scheduled task)
- `Quiz/backend/run-backup-cleanup.ps1` (backup + retention cleanup)
- `Quiz/backend/restore-from-backup.ps1` (safe restore from selected backup)
- `Quiz/backend/setup-monitoring-tasks.ps1` (health/error/disk scheduled monitoring)
- `Quiz/backend/monitor-health.ps1` (health checks + alerts.log on failures)
- `Quiz/backend/monitor-errors.ps1` (5xx error-rate checks)
- `Quiz/backend/monitor-disk.ps1` (disk space alerts)
- `Quiz/backend/monitor-db.ps1` (periodic API/data performance sample logging)
