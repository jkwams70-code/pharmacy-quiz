...
function parseSource(content) {
  const sandbox = { window: { MEDLENS_DISEASE_DATABASE: {} } };
  vm.runInNewContext(content, sandbox, { timeout: 3000 });
  return sandbox.window.MEDLENS_DISEASE_DATABASE || {};
}
async function sourceEntries(frontendPath) {
  const file = path.join(frontendPath, "medlens-disease-database.js");
...
async function fetchWikipediaDisease(term) {
  ...
  const json = await fetchMedLensSource(endpoint, "Wikipedia", { "User-Agent": "MedLens/1.0 disease fetcher" });
  if (!json) return null;
  if (json.error) throw new Error("Wikipedia could not retrieve the disease source. Please retry.");
