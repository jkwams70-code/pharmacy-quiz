...
];async function sourceEntries(frontendPath) { return parseSource(await fs.readFile(path.join(frontendPath, "medlens-interactions-database.js"), "utf8")); }
...
function runEditor({ id, name, command, args, cwd }) { return new Promise((resolve, reject) => { const child = spawn(command, args, { cwd, env: process.env, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] }); let error = ""; child.stdout.on("data", () => progress.set(id, { status: "running", percent: 50, completed: 1, total: 1, step: "Editing interaction card...", drugName: name, error: "" })); child.stderr.on("data", (x) => { error += x.toString(); }); child.on("error", reject); child.on("close", (code) => code === 0 ? resolve() : reject(new Error(error.trim() || `AI editor exited with code ${code}.`))); }); }
export function createMedLensInteractionRouter({ config, frontendPath }) { const router = express.Router();
  ...
  router.post("/api/admin/medlens/interactions/fetch", async (req, res) => {
    ...
    for (const term of terms) {
      ...
      let labels;
      try { labels = await Promise.all(parts.map((part) => fetchLiveLabel(part))); }
      ...
      // Keep both requested drugs, even if only one source label is available.
      const drugs = parts;
    ...
    if (fetched.length) await writeCollection("medlensInteractionQueue", all);
    if (errors.length && !fetched.length) return res.status(502).json({ error: errors[0].error, errors, duplicates, notFound });
    res.json({ ok: true, fetched, duplicates, notFound, errors, total: fetched.length });
