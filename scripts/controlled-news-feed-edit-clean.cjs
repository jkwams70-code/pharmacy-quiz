...
function replaceRegexOnce(text, regex, replacement, label) {
  let count = 0;
  const next = text.replace(regex, function () { count += 1; return replacement; });
  if (count !== 1) throw new Error('Expected one replacement for ' + label + ', got ' + count);
  return next;
}
const constantsNew = `
