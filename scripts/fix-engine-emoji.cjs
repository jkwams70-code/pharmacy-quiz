...
const files = [
  'www/engine.js',
  'android/app/src/main/assets/public/engine.js',
];
...
function replaceArrayBlock(source, startMarker) {
  const start = source.indexOf(startMarker);
  if (start === -1) return source;
  const openIndex = source.indexOf('[', start);
  if (openIndex === -1) return source;
  const end = findMatchingBlockEnd(source, openIndex, '[', ']');
...
for (const rel of files) {
  ...
  source = replaceArrayBlock(source, 'const COMMUNITY_EMOJI_SKIN_TONES = [');
  source = replaceArrayBlock(source, 'const COMMUNITY_EMOJI_TONE_COMPATIBLE = new Set([');
  source = replaceArrayBlock(source, 'const COMMUNITY_CHAT_EMOJI_CATEGORIES = [');
  if (source !== original) {
